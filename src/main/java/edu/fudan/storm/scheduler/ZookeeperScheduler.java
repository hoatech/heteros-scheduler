...
public class ZookeeperScheduler implements IScheduler {
    ...
    public void schedule(Topologies topologies, Cluster cluster) {
        ...
        for (TopologyDetails topology : topologies.getTopologies()) {
            ...
            for (String key : jedisClient.hkeys(cpu_load_map)) {
                ...
                if (cpuLoad == null || cpuLoad.size() == 0) {
            ...
            try {
                ...
                InterTaskTraffic = taskPartition(taskLoadMap, topology);
            ...
            if (!this.scheduleMap.containsKey(topologyId) || !this.scheduleMap.get(topologyId).equals(schedule)) {
                ...
                LOG.info("Chosed supervisors: "+choosedSupervisors);
    ...
    private List<List<Integer>> workerPartition(String topologyName, long[][] InterTaskTraffic, Map<Integer, Integer> taskOfPartition, int numWorkers, List<Integer> partitionSize) throws IOException {
        ...
        for(Integer fromTaskId : taskOfPartition.keySet()){
            int workerId = taskOfPartition.get(fromTaskId);
            for(int toTaskId=1;toTaskId<InterTaskTraffic[0].length;toTaskId++){
                InterWorkerTraffic[workerId][taskOfPartition.get(toTaskId)] += InterTaskTraffic[fromTaskId][toTaskId];
            }
        }
        ...
        for(Map<Integer, Long> traffic : trafficList){
            LOG.info("Inter-worker traffic of "+trafficList.indexOf(traffic)+" : "+traffic);
        }
        //write header of graph
        writer.write(numWorkers+" "+linkNumber/2+" "+"011"+"\n");
        for(int i=0;i<numWorkers;i++){
            String line = "1";
    ...
    private long[][] taskPartition(Map<Integer, Long[]> taskLoadMap, TopologyDetails topologyDetails) {
        ...
        try {
            ...
            Runtime runtime=Runtime.getRuntime();
            Process proc = runtime.exec(new String[]{Stootils.metis_binary, graph_file, Integer.toString(num_worker), "-ptype=rb"});
            BufferedReader METIS_reader = new BufferedReader(new InputStreamReader(proc.getInputStream()));
            String message;
            while((message=METIS_reader.readLine())!=null)
                LOG.info(message);
            ...
            LOG.info("Phase 1 METIS schedule result "+schedule.split("\n"));
