    ...
    private static Map<Integer, AtomicLong> createEmptySendgraphMap() {
        return new HashMap<Integer, AtomicLong>() {
            ...
            public String toString() {
                ...
                for (java.util.Map.Entry<Integer, AtomicLong> entry : entrySet()) {
                    if (sb.length() > 0) {
                        sb.append(" ");
                    }
            ...
            */
            context.registerMetric(METRIC_EMITTED_MESSAGES, new IMetric() {
                ...
                public Object getValueAndReset() {
                    ...
                    currentValue = edu.fudan.storm.metrichook.SchedulingMetricsCollectionHook.this.sendgraphRef.get();

                    LOG.trace("Reset values for task {} and returning: {}", context.getThisTaskId(), currentValue.toString());

                    //clear send graph
                    edu.fudan.storm.metrichook.SchedulingMetricsCollectionHook.this.sendgraphRef=new AtomicReference<>();;
                    edu.fudan.storm.metrichook.SchedulingMetricsCollectionHook.this.sendgraphRef.compareAndSet(null, createEmptySendgraphMap());
