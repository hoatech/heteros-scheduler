...
package edu.fudan.storm.metrichook;
...
/**
 * This class consumes the metrics that are collected by the {ch.uzh.ddis.stools.scheduler.SchedulingMetricsCollectionHook}
 ...
 * for { ch.uzh.ddis.stools.scheduler.SchedulingMetricsToZookeeperWriter#DEFAULT_SCHEDULING_METRIC_TIMEOUT_SECS}
 ...
 * { ch.uzh.ddis.stools.scheduler.SchedulingMetricsCollectionHook#getConfiguredSchedulingIntervalSecs(Map)}
 * seconds.
 *
...
public class SchedulingMetricsToZookeeperWriter implements IMetricsConsumer {
    ...
    /**
     * We store the interval in which the tasks hooks send data to the metrics framework, so we can issue an update
    ...
    public void prepare(Map stormConf, Object registrationArgument, TopologyContext context,
        ...
        try {
            if (zkClient.checkExists().forPath(this.zookeeperSendgraphPath) == null) {
        ...
        this.dateUpdateTask = new Runnable() {
            @Override
            public void run() {
                LOG.info("Updating sendgraph in ZK for path {}", zookeeperSendgraphPath);
                try {
    ...
    public void handleDataPoints(TaskInfo taskInfo, Collection<DataPoint> dataPoints) {
        ...
        for (DataPoint dp : dataPoints) {
            ...
            if (SchedulingMetricsCollectionHook.METRIC_EMITTED_MESSAGES.equals(dp.name)) {
                LOG.info("Datapoint of task {}: {}", taskId, dp.value.toString());
